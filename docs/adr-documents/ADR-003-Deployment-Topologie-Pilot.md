...
- **Konkretisierung Migrations- und Retention-Rolle:** Kein ausgeliefertes Profil enthält die `*_migrator`-Verbindungen; nach ADR-001 §2.4 liegt die Migrationsrolle nie in einem Runtime-Container. Schema-Migrationen laufen als separater Ops-Schritt über den einmaligen `migrate`-Service, lokal über `full`. Offen: Der Retention-Job (`elyo:enforce-retention`) braucht `health`, `identity` und `audit_migrator`. `audit_migrator` ist erforderlich, weil die Audit-DB für alle Runtime-Rollen append-only ist — das Pruning kann daher nur die Migrator-Rolle ausführen. Damit ist der Job strukturell an die noch nicht ausgelieferte Privacy/Admin-Runtime gebunden und **kann nicht** im `identity`-, `employee`- oder `company`-Profil laufen. Der Scheduler-Eintrag in `routes/console.php` bleibt bis dahin auskommentiert; lokal läuft der Job im `api-tooling`-Container. Vor Aktivierung ist zu entscheiden, ob die Privacy-Runtime tatsächlich `audit_migrator` erhält oder eine engere `elyo_audit_pruner`-Rolle eingeführt wird.

- **Konkretisierung Identity-Verbindungen (Nachtrag ELYO-106):** Das `identity`-Profil führt genau eine Verbindung (`identity`). Der ursprüngliche `audit`-Eintrag wurde entfernt: `elyo_identity_rt` besitzt kein CONNECT auf `elyo_audit`, und nach der Entkopplung der Invite-Provisionierung (siehe D5-Nachtrag) schreibt die Identity-Runtime keine Audit-Events. Eine Allowlist beschreibt, was eine Runtime tatsächlich erreichen kann — nicht, was sie einmal brauchen könnte; der Eintrag kehrt gemeinsam mit dem Grant und dem schreibenden Code zurück. Das `company`-Profil behält `audit`, weil `elyo_company_rt` CONNECT und INSERT besitzt.
...
- **Nachtrag ELYO-106 — Provisionierungszeitpunkt (ersetzt die synchrone Invite-Provisionierung):** Ursprünglich provisionierte `InviteAcceptanceService` das Health-Subject synchron direkt nach dem Identity-Commit. Der Runtime-Split hat das als Grenzverletzung sichtbar gemacht: `/api/auth/invite/accept` wird von der Identity-Runtime bedient, die weder eine `mapping`-Verbindung noch Mapping-Schlüsselmaterial besitzt — durch die Konstruktor-Injektion des Mapping-Service scheiterten dort sogar `verify` und `accept` mit HTTP 500. Die Provisionierung erfolgt daher jetzt ausschließlich dort, wo die Credentials legitim vorliegen: `App\Services\Health\ResolvesOwnSubject` provisioniert idempotent beim ersten Health-Zugriff in der Employee-Runtime, `elyo:provision-subjects` gleicht im Bulk ab. Fachlich ist das kein Verlust — die Provisionierung war schon vorher als „best effort mit Reparatur-Sweep" spezifiziert — und es ist datensparsamer: Wer nie ein Health-Feature öffnet, erhält keinen Health-Domain-Datensatz. Kein Queue-Worker und keine zusätzliche Runtime sind dafür nötig.
