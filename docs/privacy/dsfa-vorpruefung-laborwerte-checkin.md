...
| # | Artefakt | Dateireferenz (Branch: `demo/employee-lab-values-dashboard`) | Befund | Demo-Ist | Restrisiko Ziel (geplant) | Adressiert durch | Umsetzungsstatus |
|---|---|---|---|---|---|---|---|
| R1 | `lab_markers`-Tabelle | `apps/api-laravel/database/migrations/2026_07_05_010000_create_lab_markers_table.php` | Direkter `user_id`-FK (cascade), `unique(user_id, marker_key)`, keine Historie, kein Retention-Konzept | **Blocker** | Low — Neubau am `health_subject_id` | ADR-001 §2.1/2.6; ELYO-104/105 (offen) | **Technisch nachgewiesen:** `docker compose exec api-tooling php artisan test --testsuite=boundary` → `HealthSchemaBoundaryTest`; Health-Migrationen und `App\Models\Health\LabMarker*`; Prompts 03/08/08a/11, commits `498125e`, `162bc85`, `efab7ca`, `6ca5a26`. |
| R2 | `LabMarker`-Model + Relation | `app/Models/LabMarker.php`, `User::labMarkers()` | Identity↔Health-Join per ORM trivial | **Blocker** | Low — Model wird nicht übernommen; Boundary-Tests | ADR-001 §2.10; ELYO-106/111 (offen) | **Technisch nachgewiesen:** Boundary-Suite → `SourceBoundaryTest` und `SubjectMappingStorageBoundaryTest`; Privacy-Suite → `MappingNonJoinabilityPrivacyTest`; Prompts 04/06/16, commits `2b73ba2`, `1d3730a`, `f58c16e`. |
| R3 | Lab-Endpoint nur route-gesichert | `app/Http/Controllers/Employee/LabMarkerController.php` | Schutz nur auf Route-Ebene; keine Schema-/Service-Grenze | **High** | Low — DB-Rollen-Trennung macht Joins technisch unmöglich | ADR-001 §2.4; ELYO-106 (offen) | **Technisch nachgewiesen:** Boundary-Suite → `PostgresRoleBoundaryTest`; Privacy-Suite → `LabAccessPrivacyTest`; `bash infra/smoke-runtime-split.sh`; Prompts 06/14/15/16, commits `1d3730a`, `8370a59`, `e62628b`, `d5e8fe6`. |
...
**Blocker-Zusammenfassung (steuert ELYO-91, ADR-002 §2.7):** Der technische Teil (a) ist für R1–R3 durch Boundary-/Privacy-Suiten und Runtime-Smoke nachgewiesen; die ursprünglichen Klassifikationen bleiben zur Nachvollziehbarkeit unverändert. Produktive Persistenz realer Gesundheitsdaten bleibt formal geblockt, bis zusätzlich (b) die Datenschutz-Verantwortliche diese Vorprüfung ohne Veto zur Kenntnis genommen hat. Einwände erzeugen neue Blocker-Einträge in dieser Tabelle.
